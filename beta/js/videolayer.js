window.onload = function() {
	...
	function vl_getAllSources(num) {
		...
		for(var j = 0, n = allElements.length; j < n; j++) {
			...
			datadefault = allElements[j].getAttribute('data-default');
			if(datanum == basenum) {
				if(datahd == 'hd') {
					vl_sources[num][datanum].hd.push(allElements[j]);
					if(datadefault == 'default') vl_default[num] = [datanum, 'hd'];
				}
				else {
					vl_sources[num][datanum].sd.push(allElements[j]);
					if(datadefault == 'default') vl_default[num] = [datanum, 'sd'];
				}
			}
			else {
				basenum++;
				vl_sources[num][basenum] = { 'sd' : [], 'hd' : [], 'tracks' : [] };
				if(datadefault == 'default') vl_default[num] = datanum;
				if(datahd == 'hd') {
					vl_sources[num][datanum].hd.push(allElements[j]);
					if(datadefault == 'default') vl_default[num] = [datanum, 'hd'];
				}
		...
		for(var k = 0, n = allTracks.length; k < n; k++) {
			...
			datadefault = allTracks[k].getAttribute('data-default');
			if(datanum == basenum) {
				vl_sources[num][datanum].tracks.push(allTracks[k]);
				if(datadefault == 'default') vl_track_def[num][datanum] = tracknum;
				tracknum++;
			}
			else {
				basenum++;
	...
	function vl_getIfHD(newSrc, num) {
		...
		if(newSrc.sd[0].getAttribute('data-repeatable') == 'repeatable') {
	...
	function vl_countSources(num) {
		...
		while(i < allElements.length) {
	    	if(allElements[i].getAttribute('data-num')) {
	    		...
	    		if(allElements[i].getAttribute('data-default')) {
	    			if(allElements[i].getAttribute('data-default') == 'default') {
	    				num = datanum;
	    				num++;
	    			}
	    		}
    ...
    function vl_addListeners(i) {
    	...
    	vl_video[i].addEventListener('ended', function() { vl_endedVideo(i); }, false);
    	...
    	vl_ui[i].container.addEventListener('mouseup', vl_addKeysDown, false);
    ...
    function vl_repeatAndShuffleMouseUp(element) {
    	...
    	if(element.className == 'vl_playlist_shuffle') {
